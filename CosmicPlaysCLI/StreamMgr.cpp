...
bool StreamMgr::InitVideo()
{
	...
	ovi.base_width = opt.screenX;
	ovi.base_height = opt.screenY;
	ovi.output_width = opt.outputX;
	ovi.output_height = opt.outputY;
...
bool StreamMgr::StartStreaming()
{
	...
	obs_data_set_int(h264Settings, "bitrate", opt.bitrate);

	obs_data_t* aacSettings = obs_data_create();

	obs_data_set_string(aacSettings, "rate_control", "CBR");
	obs_data_set_int(aacSettings, "bitrate", opt.audioBitrate);
...
bool StreamMgr::StartRecording()
{
	...
	switch(opt.mode) {
		case 1:
			streamOutput = obs_output_create("ffmpeg_muxer", "File output", settings, nullptr);
			obs_data_set_string(settings, "muxer_settings", opt.muxerFlags);
			break;
		case 2:
...
void StreamMgr::StopRecording()
{
	try {
		obs_output_end_data_capture(streamOutput);
